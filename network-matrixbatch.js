/**
 * Much slower than the other network because of slow matrix calculation
 */
...
class Network {
    ...
    feedForward(input) {
        if (Array.isArray(input[0])) {
            let activations = Matrix.transpose(input);
            for (let [w, b] of zip(this.weights, this.biases))
                activations = sigmoid(Matrix.add(Matrix.dot(w, activations), Matrix.transpose([b])));
            return Matrix.transpose(activations);
        } else {
    ...
    stochasticGradientDescent(trainingData, epochs, miniBatchSize, eta, testData, callback) {
        ...
        for (let epoch of range(epochs)) {
            ...
            for (let i of range(0, trainingData.length, miniBatchSize)) {
                ...
                //     Vector.subtract(b, Matrix.multiply(nb, rate)));
                // this.weights = zip(this.weights, nablaW).map(([w, nw]) =>
                //     Matrix.subtract(w, Matrix.multiply(nw, rate)));
            }
    ...
    backpropagate(inputs, desiredOutputs) {
        ...
        // Feed forward
        let activations = Matrix.transpose(inputs);
        ...
        let delta = Matrix.multiply(costPrime, sigmoidPrime(activations));
        nablaB.unshift(delta.map(row =>
            row.reduce((sum, value) => sum + value, 0)));
        nablaW.unshift(Matrix.map(
            new Matrix([delta.length, layers[layers.length - 2].length]),
            (_, [row, col]) =>
                Matrix.multiply(delta[row], layers[layers.length - 2][col])
        ...
        for (let l = 2; l < this.numLayers; l++) {
            ...
            nablaW.unshift(Matrix.map(
                ...
                (_, [row, col]) =>
                    Matrix.multiply(delta[row], layers[layers.length - l - 1][col])
                        .reduce((sum, v) => sum + v, 0)));
        }
